...
def generate_insightful_summary(df: pd.DataFrame, file_name: str = "Uploaded Dataset") -> str:
    ...
    if missing_count == 0:
        ...
        lines.append(f"- **Data Quality:** {missing_count} missing values ({missing_percent:.1f}% of total data)")
...
# Validate file
if file_uploader.size > 200 * 1024 * 1024:
    st.error("File too large. Please upload a file < 200MB.")
    st.stop()
